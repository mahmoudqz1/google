  ...
  methods: {
    async fetchLeads() {
      try {
        ...
        this.leads.sort((a, b) => b.date - a.date)
      }
      catch (error) {
    ...
    >
      <VCard>
        <VCardTitle class="headline grey lighten-2">
          {{ formMode === 'add' ? 'Ajouter un lead' : formMode === 'edit' ? 'Modifier le lead' : 'Détails du lead' }}
        </VCardTitle>
        <VCardText>
          <VForm v-model="valid">
            <VContainer>
              ...
              <template v-if="formMode === 'details'">
                <VRow>
                  ...
                  <VCol
                    ...
                    md="6"
                  >
                    <strong>Date & Heure:</strong> {{ formatDateTime(lead.date) }}
                  </VCol>
                </VRow>
              </template>
        ...
        <VCardActions>
          ...
          >
            {{ formMode === 'add' ? 'Ajouter le lead' : 'Mettre à jour le lead' }}
          </VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
